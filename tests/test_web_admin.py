...
def test_web_admin_contains_py_topology_and_notice_controls() -> None:
    ...
    assert 'data-confidence="${esc(confidence)}"' in version_line
    for confidence in ('local', 'direct', 'reported'):
        assert f'.tag[data-confidence="{confidence}"]' in text
    assert version_line.index('esc(confidence)') < version_line.index('row.protocol_version') < version_line.index('row.software_version')
    assert '<strong>Health</strong> ${esc(healthLabel)}' in text
    ...
    assert '.known-node-table th:first-child,.known-node-table th:last-child{width:1%;white-space:nowrap}' in text
