...
| Spots and RBN spots | spot tables plus ingest metadata | `dx`, `show/dx`, `show/mydx`, `show/rbn` | public Spots and Watch views | RBN is not a peer family. It is a feed and spot source controlled by global/node settings plus per-user preferences and filters. `show/dx` is the traditional non-RBN history; `show/rbn` is the summarized RBN history. |
